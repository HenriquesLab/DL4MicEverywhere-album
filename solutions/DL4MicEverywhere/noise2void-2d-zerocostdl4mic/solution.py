...
# Based on https://github.com/HenriquesLab/DL4MicEverywhere/blob/main/notebooks/ZeroCostDL4Mic_notebooks/Noise2Void_2D_DL4Mic/configuration.yaml
...
def install():
    ...
    notebook_path = get_app_path().joinpath("Noise2Void_2D_ZeroCostDL4Mic.ipynb")
    ...
    python_command = ["python", ".tools/notebook_autoconversion/transform.py", "-p", f"{get_app_path()}", "-n", "Noise2Void_2D_ZeroCostDL4Mic.ipynb", "-s"]
    python_command += section_to_remove

    subprocess.run(python_command, cwd=repository_path)
    subprocess.run(["mv", get_app_path().joinpath("colabless_Noise2Void_2D_ZeroCostDL4Mic.ipynb"), get_app_path().joinpath("Noise2Void_2D_ZeroCostDL4Mic.ipynb")])
...
def run():
    ...
    notebook_path = app_path.joinpath("Noise2Void_2D_ZeroCostDL4Mic.ipynb")
...
setup(
    ...
    title="noise2void-2d-zerocostdl4mic implementation.",
    ...
    tags=['AMD64', 'colab', 'notebook', 'Noise2Void', 'denoising', 'ZeroCostDL4Mic', '2D', 'dl4miceverywhere'],
    args=[{
        "name": "path",
        "type": "string",
        "default": ".",
