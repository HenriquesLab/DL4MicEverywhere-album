...
# Based on https://github.com/HenriquesLab/DL4MicEverywhere/blob/main/notebooks/ZeroCostDL4Mic_notebooks/Detectron2_2D_DL4Mic/configuration.yaml
...
def install():
    ...
    notebook_url = "https://raw.githubusercontent.com/HenriquesLab/ZeroCostDL4Mic/master/Colab_notebooks/Detectron2_2D_ZeroCostDL4Mic.ipynb"
    
    notebook_path = get_app_path().joinpath("Detectron2_2D_ZeroCostDL4Mic.ipynb")
    ...
    section_to_remove = "1.2. 2. 4.2. 6.2."
    section_to_remove = section_to_remove.split(' ')
    
    python_command = ["python", ".tools/notebook_autoconversion/transform.py", "-p", f"{get_app_path()}", "-n", "Detectron2_2D_ZeroCostDL4Mic.ipynb", "-s"]
    python_command += section_to_remove

    subprocess.run(python_command, cwd=repository_path)
    subprocess.run(["mv", get_app_path().joinpath("colabless_Detectron2_2D_ZeroCostDL4Mic.ipynb"), get_app_path().joinpath("Detectron2_2D_ZeroCostDL4Mic.ipynb")])
...
def run():
    ...
    notebook_path = app_path.joinpath("Detectron2_2D_ZeroCostDL4Mic.ipynb")
...
setup(
    ...
    name="detectron2-2d-zerocostdl4mic",
    version="1.15.1",
