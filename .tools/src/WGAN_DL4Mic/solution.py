...
# Based on https://github.com/HenriquesLab/DL4MicEverywhere/blob/main/notebooks/ZeroCostDL4Mic_notebooks/WGAN_DL4Mic/configuration.yaml
...
def install():
    ...
    notebook_url = "https://raw.githubusercontent.com/HenriquesLab/ZeroCostDL4Mic/master/Colab_notebooks/WGAN_ZeroCostDL4Mic.ipynb"
    
    notebook_path = get_app_path().joinpath("WGAN_ZeroCostDL4Mic.ipynb")
    ...
    python_command = ["python", ".tools/notebook_autoconversion/transform.py", "-p", f"{get_app_path()}", "-n", "WGAN_ZeroCostDL4Mic.ipynb", "-s"]
    python_command += section_to_remove

    subprocess.run(python_command, cwd=repository_path)
    subprocess.run(["mv", get_app_path().joinpath("colabless_WGAN_ZeroCostDL4Mic.ipynb"), get_app_path().joinpath("WGAN_ZeroCostDL4Mic.ipynb")])
...
def run():
    ...
    notebook_path = app_path.joinpath("WGAN_ZeroCostDL4Mic.ipynb")
...
setup(
    ...
    cite=[{'doi': 'https://doi.org/10.1038/s41467-021-22518-0', 'text': 'von Chamier, L., Laine, R.F., Jukkala, J. et al. Democratising deep learning for microscopy with ZeroCostDL4Mic. Nat Commun 12, 2276 (2021). https://doi.org/10.1038/s41467-021-22518-0'}, {'doi': 'https://doi.org/10.48550/arXiv.1704.00028', 'text': 'Gulrajani, I., Ahmed, F., Arjovsky, M., Dumoulin, V. and Courville, A. Improved Training of Wasserstein GANs. arXiv. 2017 doi: https://doi.org/10.48550/arXiv.1704.00028.'}],
