...
# Based on https://github.com/HenriquesLab/DL4MicEverywhere/blob/main/notebooks/ZeroCostDL4Mic_notebooks/bioimageio-pytorch/configuration.yaml
...
def install():
    ...
    notebook_url = "https://raw.githubusercontent.com/HenriquesLab/DL4MicEverywhere/main/notebooks/Bespoke_notebooks/bioimageio-pytorch/load_bioimageio_pytorch.ipynb"
    
    notebook_path = get_app_path().joinpath("load_bioimageio_pytorch.ipynb")
    ...
    section_to_remove = "1.1. 1.2."
    section_to_remove = section_to_remove.split(' ')
    
    python_command = ["python", ".tools/notebook_autoconversion/transform.py", "-p", f"{get_app_path()}", "-n", "load_bioimageio_pytorch.ipynb", "-s"]
    python_command += section_to_remove

    subprocess.run(python_command, cwd=repository_path)
    subprocess.run(["mv", get_app_path().joinpath("colabless_load_bioimageio_pytorch.ipynb"), get_app_path().joinpath("load_bioimageio_pytorch.ipynb")])
...
def run():
    ...
    notebook_path = app_path.joinpath("load_bioimageio_pytorch.ipynb")
